...
def analyze_consumption():
    ...
    try:
        ...
        print(
            f"Electricity Currently Delivered: {latest_reading.electricity_currently_delivered} kW"
        )
        print(
            f"Electricity Currently Returned: {latest_reading.electricity_currently_returned} kW"
        )

        # Check if user is generating more electricity than consuming
        if (
            latest_reading.electricity_currently_delivered == 0
            and latest_reading.electricity_currently_returned > 0
        ):
            print("\n=== Explanation ===")
            print(
                "Your smart meter is showing 0.000 kW for consumption because you are currently"
            )
            ...
            print(
                "When your solar panels produce more electricity than your home is using,"
            )
            print(
                "the excess electricity is returned to the grid, and your net consumption shows as zero."
            )
            ...
            print(
                "3. Since your generation exceeds your usage, the net consumption is zero"
            )
            print("4. The excess electricity is being returned to the grid")

            print("\n=== Suggestions ===")
            print(
                "If you want to see your actual consumption (regardless of solar production),"
            )
            print(
                "you would need additional monitoring equipment that measures consumption"
            )
            ...
            zero_consumption_count = sum(
                1 for r in recent_readings if r.electricity_currently_delivered == 0
            )
            ...
            if zero_consumption_count > 80:
                ...
                print(
                    "This is common during sunny days when solar panels are producing at their peak."
                )
                ...
                print(
                    "This might happen during peak sunlight hours when solar production is highest."
                )

        elif latest_reading.electricity_currently_delivered > 0:
            print("\n=== Explanation ===")
            print(
                "Your smart meter is correctly showing your current electricity consumption."
            )
            print(
                f"You are currently consuming {latest_reading.electricity_currently_delivered} kW."
            )

            if latest_reading.electricity_currently_returned > 0:
                print(
                    f"You are also returning {latest_reading.electricity_currently_returned} kW to the grid."
                )
                print(
                ...
                print(
                    "This could be because it's nighttime, cloudy, or you don't have solar panels."
                )
    ...
    try:
        ...
        if today_consumption.exists():
            delivered_count = sum(
                1 for c in today_consumption if c.currently_delivered > 0
            )
            returned_count = sum(
                1 for c in today_consumption if c.currently_returned > 0
            )
            total_count = today_consumption.count()

            print(
                f"Today's readings with consumption > 0: {delivered_count}/{total_count} ({delivered_count/total_count*100:.1f}%)"
            )
            print(
                f"Today's readings with return > 0: {returned_count}/{total_count} ({returned_count/total_count*100:.1f}%)"
            )
            ...
            if delivered_count == 0 and returned_count > 0:
                ...
                delivered_count < total_count * 0.2
                and returned_count > total_count * 0.8
            ):
                print(
        ...
        if yesterday_consumption.exists():
            delivered_count = sum(
                1 for c in yesterday_consumption if c.currently_delivered > 0
            )
            returned_count = sum(
                1 for c in yesterday_consumption if c.currently_returned > 0
            )
            ...
            print(
                f"Yesterday's readings with return > 0: {returned_count}/{total_count} ({returned_count/total_count*100:.1f}%)"
            )
            ...
            if delivered_count == 0 and returned_count > 0:
                ...
                print(
                    "This pattern is normal for homes with solar panels (consumption at night, return during the day)."
                )
    ...
    print("\n=== Conclusion ===")
    print(
        "The 'consumption not showing' issue is not actually an issue - it's expected behavior"
    )

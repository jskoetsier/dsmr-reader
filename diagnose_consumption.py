...
def check_latest_readings():
    ...
    try:
        ...
        print("\n=== Phase Data ===")
        print(
            f"Phase L1 Currently Delivered: {latest_reading.phase_currently_delivered_l1} kW"
        )
        print(
            f"Phase L2 Currently Delivered: {latest_reading.phase_currently_delivered_l2} kW"
        )
        print(
            f"Phase L3 Currently Delivered: {latest_reading.phase_currently_delivered_l3} kW"
        )
        print(
            f"Phase L1 Currently Returned: {latest_reading.phase_currently_returned_l1} kW"
        )
        print(
            f"Phase L2 Currently Returned: {latest_reading.phase_currently_returned_l2} kW"
        )
        print(
            f"Phase L3 Currently Returned: {latest_reading.phase_currently_returned_l3} kW"
        )
        ...
        for idx, reading in enumerate(recent_readings):
            ...
            print(
                f"  Electricity Currently Delivered: {reading.electricity_currently_delivered} kW"
            )
            print(
                f"  Electricity Currently Returned: {reading.electricity_currently_returned} kW"
            )
            print(
                f"  Phase L1 Currently Delivered: {reading.phase_currently_delivered_l1} kW"
            )
            print(
                f"  Phase L2 Currently Delivered: {reading.phase_currently_delivered_l2} kW"
            )
            print(
                f"  Phase L3 Currently Delivered: {reading.phase_currently_delivered_l3} kW"
            )
        ...
        print(
            f"Total Electricity Currently Delivered: {latest_reading.electricity_currently_delivered} kW"
        )
        print(f"Sum of Phase Power Delivered: {total_phase_power} kW")

        if (
            abs(latest_reading.electricity_currently_delivered - total_phase_power)
            < 0.01
        ):
            ...
            print(
                "  This might indicate an issue with the meter readings or configuration."
            )
    ...
    try:
        latest_consumption = ElectricityConsumption.objects.all().order_by("-read_at")[
            0
        ]
    ...
    if total_recent_count > 0:
        ...
        print(
            f"Readings with zero consumption in last 24h: {zero_delivered_percentage:.1f}%"
        )
        ...
        if zero_delivered_percentage > 90:
            ...
            print(
                "   - Incorrect DSMR version setting (currently set to {})".format(
            ...
            if (
                total_phase_power > 0
                and latest_reading.electricity_currently_delivered == 0
            ):
                print("\n2. Your meter is reporting phase power but not total power.")
                print(
                    "   This suggests a possible issue with the DSMR protocol version."
                )
